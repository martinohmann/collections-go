...
func (c *ImmutableCollection) FirstN(n int) []*Type {
	if n > c.Len() {
		return c.Copy().Items()
	}
...
func (c *ImmutableCollection) LastN(n int) []*Type {
	if c.Len()-n < 0 {
		return c.Copy().Items()
	}
...
func (c *ImmutableCollection) Remove(pos int) *ImmutableCollection {
	d := c.Copy()
...
// The result is a new collection, the original is not modified.
func (c *ImmutableCollection) InsertItem(item *Type, pos int) *ImmutableCollection {
	var zeroValue *Type
	d := c.Copy()
...
// underlying slice.
func (c *ImmutableCollection) Cut(i, j int) []*Type {
	d := c.Copy()
...
func (c *ImmutableCollection) Slice(i, j int) []*Type {
